		...
		rt.ready = new utils._Promise(function(resolve,reject){
			...
			oReq.onload = function (){
				if(oReq.status < 400){
					...
					rt.wSocket = new WebSocket(gConfig.rtURL.replace("https","wss")+"?token="+rt.context.token+"&transport=websocket&sid=" + res.sid);
			...
			oReq.open("get",gConfig.rtURL+"?token="+rt.context.token+"&transport=polling&t="+Date.now(), true);
			oReq.send();	
		});
		return rt.ready;
	}
